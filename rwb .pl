...
print "You chose Opinions, the average of color is :$opinioncolor[0]\n, and the standerd deviation of color is :$opinioncolor[1]";
...
#my $hash = $pbkdf2 -> generate($_[1]);
 # eval { ExecSQL($dbuser,$dbpasswd,
#		 "insert into rwb_users (name,password,email,referer) values (?,?,?,?)",undef,@_);};
eval {  ExecSQL($dbuser,$dbpasswd,
		 "insert into rwb_users (name,password,email,referer) values (?,?,?,?)",undef,$_[0],$_[1] ,$_[2],$_[3]);};
  return $@;
}
