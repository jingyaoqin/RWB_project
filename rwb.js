...
// division to white and to indicate that we are updating
	var color = $("#color");
        var color_choice = document.getElementById("color_choice").innerHTML;
...
// commented out...  Those might help with the project...
	...
	var datacheckbox = document.dataform.datacheckbox;
...
//
ViewShift = function() {
var cyclevalues = document.getElementById("cycles");
...
function getCycles(){
    //var cyclecheckbox = document.getElementById("cycleid");
    var cyclecheckbox = document.dataform.cyclecheckbox;
    var cyclelist = "";
    var i;
    for (i = 0; i < cyclecheckbox.length; i++){
	if (cyclecheckbox[i].checked){
	    cyclelist +="'"+cyclecheckbox[i].value +"',";
	}
    }
    return cyclelist.substring(0,cyclelist.length-1);
//return cyclecheckbox[0].checked;
}
...
function getData(){
	...
	if (datacheckbox[i].checked){
	    datalist +=datacheckbox[i].value +",";
	}
    }
    return datalist.substring(0,datalist.length-1);
//return cyclecheckbox[0].checked;
